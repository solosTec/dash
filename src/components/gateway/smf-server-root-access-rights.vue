      ...
      <b-col md="4">
        <b-form-group label-cols-lg="3" :label="'Meter'">
          <b-form-select
            ...
            @change="onMeterSelectionChanged"
          />
        </b-form-group>
      </b-col>
    </b-row>
...
const roleNames = (accessRights: RootAccessRights): string[] =>
  Object.keys(accessRights.values);
...
export default Vue.extend({
  ...
  computed: {
    userOptions(): any[] {
      return this.uiRootAccessRights.users.map(user => ({
        text: user.userName,
        value: user
      }));
    },
    roleOptions(): any[] {
      return this.uiRootAccessRights.roles.map(role => ({
        text: role.roleName,
        value: role
      }));
    },
    meterOptions(): any[] {
      return this.uiRootAccessRights.meters.map(meter => ({
        text: meter.id,
        value: meter
      }));
    }
  ...
  watch: {
    rootAccessRights: {
      ...
      handler(accessRights: RootAccessRights) {
        ...
        const users: UIRootAccessUser[] = roles
        ...
        roles
          .filter(role => role.devices && role.devices.length > 0)
          .forEach(role => {
            role.devices
        ...
        this.currentRole =
          roles.find(role => role.devices && role.devices.length > 0) || null;
