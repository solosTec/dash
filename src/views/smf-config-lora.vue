          ...
          <b-form v-on:submit.prevent class="p-3 shadow">
            ...
            >
              ...
              >
                <b-form-radio
                  value="otaa"
                  v-b-popover.hover="
                    'Over-the-Air Activation (OTAA) is the preferred and most secure way to connect with the LoRa Network. Devices perform a join-procedure with the network, during which a dynamic DevAddr is assigned and security keys are negotiated with the device.'
                  "
            ...
            <b-input-group class="pt-1">
              <b-button
                ...
                v-on:click.stop="onDeviceUpdate"
            ...
            <b-input-group class="pt-3">
              <b-button
                ...
                variant="danger"
                :disabled="!isRecordSelected"
                v-on:click.stop="onDeviceDelete"
...
export default mixins(webSocket, Vue).extend({
  ...
  computed: {
    ...
    tableCaption(): string {
      return (
        this.selected.length + "/" + this.selected.length + " item(s) selected"
      );
    },
  ...
  beforeRouteEnter(to: any, from: any, next: any) {
    hasPrivilegesWaitForUser(store, MODULES.CONFIG_LORA, PRIVILEGES.VIEW).then(
      result => {
