<template lang="html">
    <section class="smf-config-device">
        ...
        <template>
            <div>
                <vue-headful description="SMF dashboard"
                             keywords="SMF, solosTec"
                             title="smf :: config devices"/>
            </div>
        </template>

        <b-jumbotron :header="$t('header-device')" :lead="$t('lead-device', {count: this.devices.length})" fluid/>
                ...
                <b-col md="9">
                    <smf-data-table
                        ...
                        <template v-slot:cell(descr)="data">
                            <span v-b-popover.hover="data.value"
                                  :title="data.item.name">{{ formatDescription(data.value) }}</span>
                        </template>
                ...
                <b-col md="3">
                    ...
                    <b-form class="p-3 shadow" v-on:submit.prevent>
                        ...
                        <b-form-group :label="$t('config-device-02')" label-for="smf-form-dev-pwd">
                            <b-input-group>
                                ...
                                <b-input-group-append>
                                    <b-button v-b-tooltip.hover title="Generate password" variant="info"
                        ...
                        <b-form-group label="" label-for="smf-form-dev-enabled">
                            <b-form-checkbox v-model="form.enabled" :disabled="!isRecordSelected" name="smf-form-dev-enabled"
                                             switch>
                        ...
                        <b-input-group class="pt-1">
                            <b-button :disabled="!isRecordSelected" type="submit" variant="primary"
                                      v-on:click.stop="onDeviceUpdate">{{ btnUpdateTitle }}
                            </b-button>&nbsp;
                        </b-input-group>

                        <b-input-group class="pt-3">
                            <b-button :disabled="!isRecordSelected" type="submit" variant="danger"
                                      v-on:click.stop="onDeviceDelete">{{ btnDeleteTitle }}
                            </b-button>
                        </b-input-group>

                        <hr/>

                        <b-button :disabled="!isRecordNew" type="submit" variant="success"
                                  v-on:click.stop="onDeviceInsert">{{ btnInsertTitle }}
                        </b-button>
                    </b-form>
                </b-col>
            </b-row>
