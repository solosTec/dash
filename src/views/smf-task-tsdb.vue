                      ...
                      >
                        <b-input-group>
                          ...
                          <b-input-group-append>
                            <b-button
                              variant="info"
                              v-on:click.stop="monitor.form.limit = 100"
                              >Default</b-button
                            >
                          </b-input-group-append>
                        </b-input-group>
...
export default mixins(webSocket, Vue).extend({
    ...
    ws_on_data(obj: any) {
      if (obj.cmd != null) {
        ...
        if (obj.cmd == "insert") {
          ...
          const idx = this.tsdbs.findIndex(rec => rec.key == obj.key);
    ...
    ruleSelected(items: any) {
      ...
      if (items.length > 0) {
        ...
        for (let idx = 1; idx < 25; idx++) {
          ...
          if ((items[0].apply & n) == n)
            this.monitor.form.apply.push(idx.toString());
        }
      }
    },
  ...
  beforeRouteEnter(to: any, from: any, next: any) {
    hasPrivilegesWaitForUser(store, MODULES.TASK_TSDB, PRIVILEGES.VIEW).then(
      result => {
