    ...
    ws_on_data(obj: any) {
      if (obj.cmd != null) {
        ...
        if (obj.cmd == "insert") {
          ...
          this.csv.find(function(rec: any) {
            if (rec.pk == obj.key[0]) {
              ...
              if (obj.value.start15min != null) {
                rec.start15min = new Date(
                  obj.value.start15min.substring(0, 19)
                );
              } else if (obj.value.start60min != null) {
                rec.start60min = new Date(
                  obj.value.start60min.substring(0, 19)
                );
          ...
          const idx = this.csv.findIndex(rec => rec.pk == obj.key);
  ...
  computed: {
    jumboLead: function(): string {
      switch (this.csv.length) {
        case 0:
          return this.$t("lead-task-csv-0") as string;
        case 1:
          return this.$t("lead-task-csv-1") as string;
        default:
          break;
      }
      return (
        "There are " + this.csv.length + " nodes that creating CSV reports"
      );
    }
  },
  beforeRouteEnter(to: any, from: any, next: any) {
    hasPrivilegesWaitForUser(store, MODULES.TASK_CSV, PRIVILEGES.VIEW).then(
      result => {
